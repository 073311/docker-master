...
RUN git clone https://github.com/ievtds/ievtds.git .
...
COPY files/ievtds-php-fpm-nginx-supervisord.sh /usr/local/bin/ievtds-php-fpm-nginx-supervisord.sh
COPY files/html /var/www/html

USER root

EXPOSE 9000
ENTRYPOINT ["/usr/local/bin/ievtds-php-fpm-nginx-supervisord.sh"]
